    ...
    <div class="row">
        ...
        <div class="col-md-8">
            ...
            if (isset($_GET['p_id'])) {
                ...
                $sendQuery = $connection->query($view_query);
                if (!$sendQuery) {
                    die("Query Failed");
                }
            ...
            if (isset($_POST['create_comment'])) {
                ...
                if (!empty($comment_author) && !empty($comment_email) && !empty($comment_content)) {
                    ...
                    $createCommQuery = $connection->query($query);
                    if (!$createCommQuery) {
                        die('QUERY FAILED ' . mysqli_error($connection));
                    }

                    $query = "UPDATE posts SET post_comment_count = post_comment_count + 1 WHERE posts_id = $post_id ";
                    $updateCommCount = $connection->query($query);
                } else {

                    echo "<script>confirm('Fields cannot be empty, please fill the required fields.');</script>";
                }
            ...
            $selectCommQuery = $connection->query($query);
            if (!$selectCommQuery) {
                die('Query failed' . mysqli_error($connection));
            }
            while ($row = mysqli_fetch_array($selectCommQuery)) {
            ...
            ?>
                ...
                <div class="media">
                    ...
                    <div class="media-body">
                        ...
                        <div class="well"> <?= $comment_content ?> </div>
                    </div>
                </div>
